...
def analyze(df: pd.DataFrame) -> dict:
    ...
    if sma20 > sma50: score += 1; notes.append("Short-term trend is above the 50-day trend.")
    else: score -= 1; notes.append("Short-term trend is below the 50-day trend.")
    if rsi is not None:
        if 50 <= rsi <= 70: score += 1; notes.append("RSI supports positive momentum without being deeply overbought.")
        elif rsi > 70: notes.append("RSI is elevated; momentum may be overheated.")
        elif rsi < 30: notes.append("RSI is depressed; a reversal setup may be forming, but confirmation is needed.")
        else: notes.append("RSI is neutral, so momentum confirmation is limited.")
    ...
    if price > upper: notes.append("Price is above the upper Bollinger Band; mean-reversion risk is elevated.")
    elif price < lower: notes.append("Price is below the lower Bollinger Band; downside extension is possible.")
